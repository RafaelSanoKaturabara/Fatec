...
void main(void)
{
	...
	while(true)						// loop infinito
	{
		...
		sprintf_s(cWork, sizeof(cWork), "\tFATEC-MC Estrutura de dados B1 %02d/%02d/%04d %02d:%02d:%02d %s",
			stTime.wDay,
			stTime.wMonth,
			stTime.wYear,
			stTime.wHour,
		...
		switch(cOpcao)						// avaliar a opção escolhida
		{
			case OCUPAR_MESA:
				...
				{	// mesa está ocupada - vamos exibir a sua situação
					sprintf_s(cWork, sizeof(cWork), " desde %02d:%02d", 
			...
			case CONSULTAR_MESA:
				...
				//	ocupação da mesa
				nNumMesa = PedirMesaValida("Consultar a mesa");
				if(nNumMesa == 0) break;		// volta ao menu se desistiu...
				// testar se a mesa está desocupada
				if(!objRestaurante.VerificaSeMesaOcupada(nNumMesa, &stMesa))
				{	// mesa está desocupada e portanto não pode ser servido um produto
				...
				// mostrar os dados que vieram para stMesa
				sprintf_s(cWork, sizeof(cWork), " desde %02d:%02d", 
					stMesa.stHoraOcupacao.nHora,
					stMesa.stHoraOcupacao.nMinuto);
				cout << "Mesa: " << nNumMesa << " já está ocupada!" << endl
			...
			case LIBERAR_MESA:
				...
				// testar se a mesa está desocupada
				if(!objRestaurante.VerificaSeMesaOcupada(nNumMesa, &stMesa))
				{	// mesa está desocupada e portanto não pode ser liberada
					cout << "Mesa: " << nNumMesa << " está desocupada!" << endl;
					PAUSA;
					break;							// volta ao menu
				}
