...
private:
	...
	// Método privado que torna inativo o produto no hash table
	bool InativaNoHashTable(char *ptrEAN, int nHashCode)
	{
		CHAVE_BUSCA *ptrCorrente;								
		for(ptrCorrente = vetHashTable[nHashCode].ptrPrimeira;
			ptrCorrente; ptrCorrente = ptrCorrente->ptrProxima)
		{
			if(strcmp(ptrCorrente->cEAN, ptrEAN) == 0)
			{
...
public:
	...
	bool VerificaSeExiste(char *ptrEAN, PRODUTO *ptrStProduto, CHAVE_BUSCA *ptrStChaveBusca)
	{
		...
		{	// tenho uma chave de busca
			...
			{	// vai buscar os dados deste produto no cadastro - leitura posicional
				if(!LerPosicional(ptrCorrente->nPosicional, ptrStProduto)) // erro
					return false;
				memcpy_s(ptrStChaveBusca, sizeof(CHAVE_BUSCA), ptrCorrente, sizeof(CHAVE_BUSCA));
	...
	// Método público que inativa o produto
	bool ExcluirProduto(PRODUTO *ptrStProduto, int nPosicional)
	{
		int nHash = CalcularHashCode(ptrStProduto->cEAN);	// calcular o Hash Code desse produto
		ptrStProduto->cAtivo = 'I';							// produto ativo
		if(!GravaPosicional(nPosicional, ptrStProduto))	// erro de gravação?
			return false;									// avisa que tem erro
		if(!InativaNoHashTable(ptrStProduto->cEAN, nHash)) // erro na inserção?
			return false;
		return true;										// avisa que tudo OK
	}
	// Método público que vende um produto
	bool VenderProduto(PRODUTO *ptrStProduto, int nPosicional)
	{
		if(!GravaPosicional(nPosicional, ptrStProduto))
			return false;
		return true;
	}
	...
	bool ListarCodigosDeHashCode(int nHash)
	{
		...
		cout << endl << "\tHash code " << nHash << " Qtde: " << nQtde << endl;
	}
};
