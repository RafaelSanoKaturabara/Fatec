...
public:
	...
	clRestaurante()
	{
		...
		for (i = 0; i < QTDE_PRODUTOS; i++)
		{
			...
			strcpy(tabProdutos[i].cDescricao, &cNomes[i][0]);
		...
		for(i = 0; i < QTDE_MESAS; i++)
		{
			...
			tabMesas[i].ptrPrimeiro = 0;				// não tem lista de produtos
		}
	} // construtor
	...
	bool InsereNovoProdutoNaMesa(int nMesa, int nCodProduto, int nQtdeProd)
	{
		...
		if(tabMesas[nMesa - 1].ptrUltimo == NULL)
			return false;
		tabMesas[nMesa - 1].ptrUltimo->nCodigoProduto = nCodProduto;
		tabMesas[nMesa - 1].ptrUltimo->nQuantidade = nQtdeProd;
		tabMesas[nMesa - 1].ptrUltimo->dValorProduto = tabProdutos[nCodProduto - 1].dPrecoUnitario * nQtdeProd;
	...
	bool OcuparMesa(int nMesa, int nQtdePessoas)
	{
		SYSTEMTIME stTime;
		GetLocalTime(&stTime);
		if(tabMesas[nMesa - 1].flgMesaLivreOuOcupada == true)
			return false;
		tabMesas[nMesa - 1].flgMesaLivreOuOcupada = true;
		tabMesas[nMesa - 1].nQtdePessoas = nQtdePessoas;
		tabMesas[nMesa - 1].stHoraOcupacao.nHora = stTime.wHour;
		tabMesas[nMesa - 1].stHoraOcupacao.nMinuto = stTime.wMinute;
		tabMesas[nMesa - 1].dValorTotal = 0.0;
		tabMesas[nMesa - 1].ptrPrimeiro = NULL;
		return true;											// avisa tudo OK
	}
