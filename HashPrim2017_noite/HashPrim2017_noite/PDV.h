...
#define PERCENTUAL_DE_TESTE					50					// percentual de criação de produtos
																// para testes
...
private:
	...
	//	Parâmetros:
	//		Entrada: char *ptrEAN - ponteiro para um sting que contem 
	//								o EAN do produto
	...
	//	Parâmetros:
	//		Entrada: int nHash - shash code do EAN (chave) que vai ser inserido
	//				 char *ptrEAN - código do produto que é a chave de busca
	...
	//	Parâmetros:
	//		Entrada: int - nPocicao - posição relativa desse produto detro do cadastro
	//				 PRODUTO *ptrStProduto - ponteiro de uma struct que tem os dados do produto a ser gravado
	//		Retorno: bool - true - leitura ok
	//						false - erro de seed ou de gravação
	...
	clPontoVenda(bool *ptrStatus)
	{
		...
		{	// arquivo não existe - primeira vez em que o programa é executado
			...
			for(i = 0; i < QTDE_MAX_PRODUTOS; i++)
			{
				...
				{
					fclose(fdProduto);
					*ptrStatus = false;					// avisa que teve problema
...
#ifdef TESTE
			// gerar produto para melhor testar o programa
			...
			{	// loop de criação de produtos
				...
				// produto não existe. Vai ser inclido
				sprintf_s(stProduto.cDescricao, 40, "Produto de código %s",
...
//#else
		...
		{	// o arquivo de produtos já existe
			...
			while(true)									// loop infinito
			{
				...
				{	// ler o proximo produto e considerar o seu posicional
					...
					continue;							// continua o while e lê o próximo
				}
				...
				nProxProduto++;							// incrementa para o próximo produto
			} // while
	...
	//										 recber os dados do produto se ele existir
	//		Retorno: bool - true - o produto existe e seua dados foram copiados
	//							   para o invocador do método
	//						false - o produto não existe
	bool VerificaSeExiste(char *ptrEAN, PRODUTO *ptrStProduto)
	{
		CHAVE_BUSCA *ptrCorrente;						// aponta para a chave corernte
	...
	bool IncluirNovoProduto(PRODUTO *ptrStProduto)
	{
		...
		if(!InsereNoHashTable(nHash, ptrStProduto->cEAN, nProxProduto))	// erro na inserção:
			return false;
		nProxProduto++;										// posiciona para o próximo produto
	...
	//						false - não tem chaves de busca porque o hash code está vazio
	bool ListarCodigosDeHashCode(int nHash)
	{
		int nQtde = 0,										// quantidade de chaves encontradas
			nLinha = 0;										// contator por linha
		CHAVE_BUSCA *ptrCorrente;							// chave de busca corrente
		if(vetHashTable[nHash].ptrPrimeira == NULL)			// lista vazia?
			return false;									// retorna indicando vazia
		for(ptrCorrente = vetHashTable[nHash].ptrPrimeira;
			ptrCorrente;									// nulo é fim da lista
			ptrCorrente = ptrCorrente->ptrProxima)			// indexa para a próxima chave
